...
function setupQuiz() {
    // Wizard de etapas
    const etapaModo = document.getElementById('etapa-modo');
    const etapaFeedback = document.getElementById('etapa-feedback');
    const etapaIniciar = document.getElementById('etapa-iniciar');
    const modoOpcoes = etapaModo.querySelectorAll('.modo-opcao');
    const feedbackOpcoes = etapaFeedback.querySelectorAll('.feedback-opcao');
    const btnIniciar = document.getElementById('iniciar-simulado');
    const wizardQuiz = document.getElementById('wizard-quiz');
    const quizCard = document.getElementById('quiz');
    let modoSelecionado = false;
    let feedbackSelecionado = false;

    // Esconde etapas 2 e 3 no início
    etapaFeedback.classList.add('escondido');
    etapaIniciar.classList.add('escondido');
    btnIniciar.disabled = false;

    // Função para rolar até a etapa
    function scrollToEtapa(etapa) {
        setTimeout(() => {
            etapa.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }, 200);
    }
    ...
    modoOpcoes.forEach(opcao => {
        opcao.addEventListener('click', () => {
            modoOpcoes.forEach(op => op.classList.remove('selecionado'));
            opcao.classList.add('selecionado');
            modoDesafio = opcao.dataset.modo === 'desafio';
            modoSelecionado = true;
            // Avança para etapa 2 automaticamente
            etapaFeedback.classList.remove('escondido');
            etapaIniciar.classList.add('escondido');
            feedbackOpcoes.forEach(op => op.classList.remove('selecionado'));
            feedbackSelecionado = false;
            scrollToEtapa(etapaFeedback);
        });
    });

    // Etapa 2: Seleção do tipo de feedback
    feedbackOpcoes.forEach(opcao => {
        opcao.addEventListener('click', () => {
            ...
            // Avança para etapa 3 automaticamente
            etapaIniciar.classList.remove('escondido');
            scrollToEtapa(etapaIniciar);
        });
    });

    // Etapa 3: Iniciar simulado
    btnIniciar.addEventListener('click', async () => {
        try {
            wizardQuiz.style.display = 'none';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const btnQuiz = document.querySelector('.btn-quiz-start');
    if (btnQuiz) {
    ...
    document.querySelectorAll('.btn-estudar').forEach(btn => {
        if (btn.textContent.trim() === 'Estudar') {
